  ...
  console.log("hello world");
    if(req.query.query){
        ...
        const allListing = await Listing.find({
            $or: [
                { title: { $regex: searchQuery, $options: "i" } }, // Match title
                { location: { $elemMatch: { $regex: searchQuery, $options: "i" } } } ,// Match location array elements
...
module.exports.createListing = async (req, res) => {
  console.log("createListing function",req.files.url);
  let result = await geocodingClient
    .forwardGeocode({
      query: req.body.listing.location.join(", "),
      limit: 1,
    })
    .send();
...
module.exports.showListing = async (req, res) => {
  let { id } = req.params;
  const ind_listing = await Listing.findById(id)
...
module.exports.renderEditForm = async (req, res) => {
  ...
  if (!E_listing) {
    ...
    const originalUrl = E_listing.image[0].url;
